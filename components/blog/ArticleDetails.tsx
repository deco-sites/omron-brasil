...
import BannerItemSideToSide, {
  type BannerItemProps,
} from "$store/components/blog/BannerSideToSide.tsx";
import RelatedBlogPosts, {
  type RelatedBlogPostsProps,
} from "$store/components/blog/RelatedBlogPosts.tsx";
...
export default function ArticleDetails(
  { banner, post, relatedPosts }: ArticleDetailsProps,
) {
  return (
    <section class="flex flex-col w-full h-full bg-white">
      ...
      <div class="grid lg:grid-cols-[300px_1fr] gap-y-12 lg:gap-y-0 gap-x-8 w-full h-full text-center lg:items-start lg:text-start py-20 px-6 md:px-12 pb-14">
              ...
              <button
                onClick={() =>
                  navigator.clipboard.writeText(
                    "https://deco-sites-omron-brasil.deno.dev/1/blog",
                  )}
                aria-label="Sharing Item"
                class="px-4 py-3 flex items-center justify-center border border-black font-bold h-[48px] max-w-[90px] bg-transparent rounded-xl"
              >
                <svg
                  width="18"
                  height="19"
        ...
        <div class="w-full h-full">
          <div class="flex w-full gap-8 text-start items-center justify-center pb-16">
            <ArticlePost
              image={post?.image}
              graphicImage={post?.graphicImage}
            />
          </div>
        </div>
      </div>
      ...
      <footer class="flex flex-col w-full h-full pb-6 px-6 md:px-12">
        <div class="flex flex-col gap-6 items-start border-y border-y-gray py-7 w-full h-full">
          ...
          <p>
            Williams B, Giuseppe M, Spiering W, et al. (2018). 2018 ESC/ESH
            Guidelines for the management of arterial hypertension. Journal of
            Hypertension, 36(10). doi: 10.1097/HJH.0000000000001940
          </p>
