...
function Button({ title, background, arrow }: ButtonProps) {
  const buttonConfig = background === "border-none" ? 'border-black border text-black' : 'bg-[#005EB8] text-white';

  return (
    <button className={`${buttonConfig} font-bold rounded-xl w-full md:max-w-[300px] flex p-2 items-center justify-center gap-1`}>
      {arrow === "left" && (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <g clip-path="url(#clip0_9112_20540)">
            <path d="M21.0002 11.1998H5.7002L11.0002 5.8998L9.9002 4.7998L3.3002 11.3998C3.0002 11.6998 3.0002 12.1998 3.3002 12.4998L9.9002 19.0998L11.0002 17.9998L5.7002 12.6998H21.0002V11.1998Z" fill="black"/>
          </g>
          <defs>
      ...
      <div class="grid lg:grid-cols-[300px_1fr] gap-y-12 lg:gap-y-0 gap-x-8 w-full h-full text-center lg:items-start lg:text-start py-20 px-6 md:px-12">
            ...
            <div class="hidden lg:flex flex-col border-l border-l-[#F2F2F2] gap-8">
              {topics?.map((item) => (
                <div
                  ...
                  class={`${selectedTopics === item && 'border-l-2 border-l-[#005EB8] font-bold'} pl-5 cursor-pointer`}
                >
                  {item.label}
                </div>
              ))}
        ...
        <div class="w-full h-full">
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 w-full gap-8 text-start items-center justify-center pb-16">
            {selectedTopics?.posts?.map((topics) => <ArticleCard {...topics} />)}
          </div>

          <div class="flex flex-col gap-4 md:flex-row items-center justify-between border-t border-t-[#C9C9C9] pt-12 w-full">
            <Button title="Health and Lifestyle main page" background="border-none" arrow="left" />
